...
public:
                 ...
                 const std::string &routingKey = "routing_key")
    {
        ...
        _channel->declareExchange(exchange, AMQP::ExchangeType::direct)
            .onError([](const char *message){ ERROR("声明交换机失败 : {}", message); })
            .onSuccess([](){ DEBUG("声明交换机成功"); }); 
        // 声明队列并设置声明成功与失败的回调函数
        _channel->declareQueue(queue)
            .onError([](const char *message) { ERROR("声明队列失败 : {}", message); })
            .onSuccess([](){ DEBUG("声明队列成功"); });
        // 绑定交换机和队列
        _channel->bindQueue(exchange, queue, routingKey)
            .onError([](const char *message) { ERROR("绑定交换机和队列失败 : {}", message); })
            .onSuccess([]() { DEBUG("绑定交换机和队列成功"); });
    }
                 ...
                 const MessageCallback &cb)
    {
        _channel->consume(queue)
            ...
            .onError([](const char *message){ ERROR("订阅队列消息失败 : {}", message); return false; }); 
