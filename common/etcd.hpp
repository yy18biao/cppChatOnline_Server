...
namespace hjb
{
    ...
    public:
        ...
        EtcdRegClient(const std::string &host)
            : _client(std::make_shared<etcd::Client>(host)), _keep(_client->leasekeepalive(3).get()), _leaseId(_keep->Lease())
        {
        }
    ...
    public:
        EtcdDisClient(const std::string &host,
            ...
            : _client(std::make_shared<etcd::Client>(host)), _putCB(putCB), _deleteCB(deleteCB)
        {
            // 进行服务发现，获取当前已有数据
